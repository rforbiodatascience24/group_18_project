...
Then, we select the count data and generate a table with gene expression. We also rename the entry raw_counts to counts in order to be able to normalize data afterwards.

If you get an error running the following code block you have to clear your R environment and rerun the previous code. We are not sure about the reason, but recount has sometimes weird behavior :)
...
write_csv(tpm_counts,file.path(raw_data_path,"01_BRCA_tpm_counts.csv"))
write_csv(meta_data, file.path(raw_data_path,"01_BRCA_metadata.csv"))
write_csv(counts, file.path(raw_data_path,"01_BRCA_counts.csv"))
