...
#deseq <- read_csv(file = file.path(processed_data_path, "deseq_results.csv"))
```

```{r}
#| echo: false
#| message: false
data_females <- read_csv(file = "02_norm_counts_filtered_female.csv")

metadata_females <- read_csv(file = "02_female_metadata.csv")

deseq <- read_csv(file ="deseq_results.csv")
```
...
```{r}
"gene_id" %in% colnames(data_females)
which(colnames(data_females) == "gene_id")
```
...
filtered_data_females <- data_females |>
  select(any_of(significant_genes$gene))
```
...
The PCA results indicate that the first few components explain most of the variance, suggesting dimensionality reduction is effective. The loadings highlight key genes driving variance, and the scores reveal sample patterns, useful for clustering or visualization in reduced-dimensional space.
...
```{r}
pca_variance <- tibble(
  PC = seq_along(pca_females$sdev),
  Variance = pca_females$sdev^2,
  Percent = Variance / sum(Variance),
  Cumulative = cumsum(Percent)
)


num_components <- pca_variance_tidy |> 
  filter(Cumulative >= 0.85) |> 
  slice(1) |> 
  pull(PC)
...
# Select top contributing variables for the first 5 PCs
top_contributing_variables <- loadings_females |> 
  group_by(Principal_Component) |> 
  slice_max(abs(Loading), n = 10) |> # Keep only the top 20 contributing variables
  ungroup()

# Plot the Heatmap
ggplot(top_contributing_variables, aes(x = Principal_Component, y = Variable, fill = Loading)) +
  geom_tile() +
...
```{r}
ggplot(top_genes_loadings_females, aes(x = reorder(term, PC1), y = PC1)) +
...
```{r}
ggplot(top_genes_loadings_females_2, aes(x = reorder(term, PC2), y = PC2)) +
...
ggplot(pca_scatter_data, aes(.fittedPC1, .fittedPC2, color = cancer_status)) + 
  geom_point(size = 1.5) +
  scale_color_manual(
